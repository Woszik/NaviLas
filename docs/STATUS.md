...
|-------|--------|----------------|-------------|
| **Nightly** | GitHub (prerelease `nightly`) | 0.5.77-nightly | 90 |
...
- **Trasa:** poprawka eksportu GPX — brakująca ścieżka `route/` w `file_paths.xml` wywalała apkę przy „Udostępnij trasę (GPX)”.

## Nightly 0.5.76

- **Planowanie trasy:** import GPX z OsmAnd (share/„Otwórz w”) → tryb **Trasa**, pas wyszukiwania po obu stronach, waypointy i cel z ręki (tap → arkusz, długie przytrzymanie = cel), eksport zwrotny GPX do OsmAnd. Plan trasy przeżywa restart apki.

## Nightly 0.5.75

- **OsmAnd moto:** Krótka / Kręta na routingu OsmAnd (bez BRouter) — realny ETA moto, profile `NaviLas krótka` / `NaviLas kręta` (inne ikony). Reimport `.osf` podmienia stare profile BRouter.
...
- **Wysokość znaku na pasku = wysokość litery N** w tytule (Material3 TitleLarge 22sp → ~16 dp). `ic_toolbar_logo` przycięty do samego znaku (bez pustego tła viewportu 108), 17×16 dp.

## Nightly 0.5.72

- Ikona na pasku: osobny wektor bez tła zamiast launcherowego `ic_launcher` (108 dp).
...
Nowa ikona: świerk i N (zamiast pinezki).

## Nightly 0.5.68

Porównaj: klik nazwy → mapa, karta zostaje otwarta po powrocie na listę.

## Nightly 0.5.67

Porównaj: bez belki tytułu/X (zamykanie: scrim, wstecz); karta zostaje otwarta po obrocie ekranu.

## Nightly 0.5.66

Porównaj: karta na wysokość treści + scrim; nazwy przyklejone u góry; swipe pagera wyłączony, dopóki karta otwarta na liście.
...
Lista → Porównaj: klik w nazwę miejsca skacze na mapę; przy przewijaniu poziomym lewa kolumna etykiet zostaje widoczna.
